...
<div class="wrap about-wrap full-width-layout mesmerize-page">
    ...
    <h2 class="nav-tab-wrapper wp-clearfix">
        ...
        <?php foreach ($mesmerize_tabs as $tabID => $mesmerize_tab): ?>
            <a href="?page=mesmerize-welcome&tab=<?php echo $tabID; ?>" class="nav-tab <?php echo($tabID === $currentTab ? 'nav-tab-active' : '') ?>"><?php echo $mesmerize_tab['title']; ?></a>
            <?php $first = false; ?>
        <?php endforeach; ?>
    </h2>
    ...
    <div class="tab-group">
        <div class="tab-item tab-item-active">
            <?php require $mesmerize_tabs[$currentTab]['partial']; ?>
        </div>
    </div>
</div>
